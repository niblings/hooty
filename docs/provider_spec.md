...
Hooty は以下の 5 つの LLM プロバイダをサポートする:
...
- **GPT シリーズ（Azure OpenAI）**: tiktoken `o200k_base` は正しいトークナイザのため、乖離なし
...
- GPT シリーズ（Azure OpenAI）や Bedrock の非 Claude モデルには補正不要（tiktoken が正確、または乖離が未測定）
...
- **他の全プロバイダ（Azure OpenAI, Azure AI Foundry, Ollama）はサーバー側で自動的にキャッシュが適用される** — コード変更・設定不要
...
LiteLLM から抽出するプロバイダ: `anthropic`, `bedrock`, `azure`, `azure_openai`。`ollama` は手動管理。
