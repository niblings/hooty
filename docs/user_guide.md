...
| ⚡ | シェルコマンド実行（run_shell, run_powershell） |
...
```
  ● global.anthropic.claude-sonnet-4-6 for 3.2s | ˄12,345 ˅1,234 »8,000 | 💭 1,200 chars (💭 500 tokens) | ctx 6%
```
...
| `for Xs` | 応答にかかった時間 |
| `˄N` / `˅N` | 入力トークン数 / 出力トークン数 |
| `»N` | キャッシュ読み取りトークン数（Prompt Caching 有効時） |
| `💭 N chars` / `💭 N tokens` | Extended Thinking の文字数・トークン数 |
...
|---|---|---|
| ● | `active` | 現在作業中のプラン |
| ✓ | `completed` | 完了し Coding モードに引き継がれたプラン |
| ◷ | `pending` | 保留中 — 後で再開する。新規プラン作成時に自動キャンセルされない |
| ✗ | `cancelled` | 破棄済み、または新しいプランに置き換えられた |

LLM はプランの CRUD 操作ツール（PlanTools）を使用してプランの作成・更新を行います。ツールの詳細は `docs/tools_spec.md` の「プラン管理ツール」セクションを参照してください。

Planning モードでの典型的なワークフロー:

1. 既存プランを確認
2. 新規プラン作成
3. ユーザーフィードバックに基づき精緻化
4. Coding モードに切り替えてプランを実装
...
`/attach capture` でウィンドウやモニターのスクリーンショットを撮影し、画像として添付できます。Windows / WSL2（PowerShell 経由で Win32 API を使用）および macOS（`screencapture` コマンド + `pyobjc-framework-Quartz` によるウィンドウ列挙）で利用可能です。

> **macOS**: ウィンドウ指定キャプチャには `pyobjc-framework-Quartz` が必要です（`pip install pyobjc-framework-Quartz` または `uv sync --extra macos-capture`）。モニタ指定のみであれば不要です。ターミナルアプリに「画面収録」権限が必要です（システム設定 > プライバシーとセキュリティ > 画面収録）。
...
| プロセス / アプリ名 | `/attach capture chrome` | プロセス名またはアプリ名（部分一致、`.exe`/`.app` サフィックス自動除去） |
...
| `/attach capture [target]` | スクリーンキャプチャで画像を添付（Windows / WSL2 / macOS） |
